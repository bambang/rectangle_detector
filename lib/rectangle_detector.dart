...
class RectangleDetector {
  ...
  /// [image] ui.Image对象
  /// 返回包含矩形特征点的RectangleFeature对象，如果没有检测到矩形则返回null
  static Future<RectangleFeature?> detectRectangleFromImage(ui.Image image) async {
  ...
  /// 返回包含矩形特征点的RectangleFeature对象，如果没有检测到矩形则返回null
  static Future<RectangleFeature?> detectRectangleFromProvider(ImageProvider imageProvider) async {
  ...
  static Future<List<RectangleFeature>> detectAllRectangles(Uint8List imageData) async {
  ...
  /// [image] ui.Image对象
  /// 返回包含所有矩形特征点的RectangleFeature对象列表
  static Future<List<RectangleFeature>> detectAllRectanglesFromImage(ui.Image image) async {
    final imageData = await _imageToBytes(image);
  ...
  /// [imageProvider] ImageProvider对象
  /// 返回包含所有矩形特征点的RectangleFeature对象列表
  static Future<List<RectangleFeature>> detectAllRectanglesFromProvider(ImageProvider imageProvider) async {
    final uiImage = await _imageProviderToImage(imageProvider);
    final imageData = await _imageToBytes(uiImage);
    return detectAllRectangles(imageData);
  }
  ...
  /// [imageProvider] ImageProvider对象
  /// 返回ui.Image对象
  static Future<ui.Image> _imageProviderToImage(ImageProvider imageProvider) async {
